...
const updateTestsFile = ({ token, adminToken, supplementId, complementId, foodId }) => {
  ...
  const updated = content
    ...
    .replace(/^@adminToken\s*=.*$/m, `@adminToken = ${adminToken || 'REPLACE_WITH_ADMIN_JWT'}`)
    .replace(/^@supplementId\s*=.*$/m, `@supplementId = ${supplementId || 'REPLACE_WITH_SUPPLEMENT_ID'}`)
    .replace(/^@complementId\s*=.*$/m, `@complementId = ${complementId || 'REPLACE_WITH_COMPLEMENT_ID'}`)
    .replace(/^@foodId\s*=.*$/m, `@foodId = ${foodId || 'REPLACE_WITH_FOOD_ID'}`);
